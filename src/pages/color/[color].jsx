...
import SideFilter from "@/components/shop/SideFilterColor";
...
export default function CategoryProduct() {
  ...
  const allData = allProducts.products.filter(
    (el) => el.color == router.query.color?.split("-").join(" ")
  );
      ...
      <main>
        <ProductLayout>
          <div className="my-4 pt-md-3 text-center">
            <h1>
            <span style={{ textTransform: "capitalize" }}>{router.query.color?.split("-").join(" ")}</span>&nbsp;цвет
            </h1>
          </div>
