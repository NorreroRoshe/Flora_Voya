...
import SideFilter from "@/components/shop/SideFilterDelivery";
...
export default function CategoryProduct() {
  ...
  const allData = allProducts.products.filter(
    (el) => el.delivery == router.query.delivery?.split("-").join(" ")
  );
      ...
      <main>
        <ProductLayout>
          <div className="my-4 pt-md-3 text-center">
            <h1>
            <span style={{ textTransform: 'capitalize' }}>{router.query.delivery?.split("-").join(" ")}</span>
