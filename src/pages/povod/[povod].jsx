...
import SideFilter from "@/components/shop/SideFilterPovod";
...
export default function CategoryProduct() {
  ...
  const allData = allProducts.products.filter(
    (el) => el.povod == router.query.povod?.split("-").join(" ")
  );
      ...
      <main>
        <ProductLayout>
          <div className="my-4 pt-md-3 text-center">
            <h1>
            <span style={{ color: 'grey' }}>Повод:</span> <span style={{ textTransform: 'capitalize' }}>{router.query.povod?.split("-").join(" ")}</span>
