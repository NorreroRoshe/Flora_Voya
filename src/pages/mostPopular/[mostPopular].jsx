...
import SideFilter from "@/components/shop/SideFilterBouquets";
...
export default function CategoryProduct() {
  ...
  const allData = allProducts.products.filter(
    (el) => el.bouquets == router.query.bouquets?.split("-").join(" ")
  );
      ...
      <main>
        <ProductLayout>
          <div className="my-4 pt-md-3 text-center">
            <h1>
            <span style={{ textTransform: 'capitalize' }}>{router.query.bouquets?.split("-").join(" ")}</span>
            </h1>
          </div>
